...
class SimpleClient(SDClient):
    ...
    def __init__(self, address, real_car_url, poll_socket_sleep_time=0.01):
    ...
    def linear(self):
        ...
        while time.time() - start_time < 0.5:
            self.send_controls(0.0, 1)

    def update_real_car(self, st, th):
        r = requests.post(self.real_car_url, json={
            "angle": st,
            "throttle": th,
            "drive_mode": "u",
            "recording": False
    ...
    def update(self, st, th, real=True):
        # just random steering now
        self.send_controls(st, th)
        if(real):
            self.update_real_car(st, th)
...
def test_clients():
    ...
    # requests.get(real_car_url)
    # Start Clients
    for _ in range(0, num_clients):
        c = SimpleClient(address=(host, port), real_car_url=real_car_url)
        clients.append(c)
    ...
    # while True:
    #   clients[0].update(0.0,0.0, False)
    ...
    # time.sleep(3.0)
    # r = requests.post(real_car_url, json={
    #     "angle": 0.0,
    #     "throttle": 0.1,
    #     "drive_mode": "u",
    #     "recording": False
